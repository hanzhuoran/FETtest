...
def generate_mat_and_geom(ring_num):    
    ...
    fuel_cell_universe = openmc.Universe(name='1.6% Fuel Cell')
    fuelcelllist = []
    for i in range(0,NumRad):
        if i == 0 :
            fuel_cell = openmc.Cell(fill=fuel,region = -fuel_radius[i])   
        else:
            fuel_cell = openmc.Cell(fill=fuel,region = -fuel_radius[i] & +fuel_radius[i-1])
        fuelcelllist.append(fuel_cell)
    #dd all cell into the unverse
    ...
    clad_cell.region = +fuel_radius[NumRad-1] & -clad_outer_radius & +min_x & -max_x & +min_y & -max_y & +min_z & -max_z
    ...
    # Add boundary planes
    total_fuel_cell.region = -fuel_radius[NumRad-1] & +min_z & -max_z
