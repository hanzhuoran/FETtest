...
def generate_xml(ring_num,porder):
    ...
    # generate_tallies(fuelcelllist,total_fuel_cell,porder,flag)
...
def generate_mat_and_geom(pln_num):    
    ...
    materials_file = openmc.Materials([fuel, water, b4c,zircaloy])
    ...
    # Create a moderator Cell at upper 2/3
    pos1 = int(pln_num/3)
    moderator_cell = openmc.Cell(name='1.6% Moderator')
    moderator_cell.fill = water
    moderator_cell.region = +clad_outer_radius & +min_x & -max_x & +min_y & -max_y & +planes[pos1] & -max_z

    #create a b4c cell at lower 1/3
    b4c_cell = openmc.Cell(name='B4C cell')
    b4c_cell.fill = b4c
    b4c_cell.region = +clad_outer_radius & +min_x & -max_x & +min_y & -max_y & +min_z & -planes[pos1]
