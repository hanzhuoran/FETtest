...
for n in N:
	...
	openmc.run(mpi_args=['mpiexec', '-n', '4'],output = False)
	#create directory
	Nstr = str(n)
	...
	filename1 = directory+'/statepoint.1100.h5'
	os.rename('statepoint.1100.h5',filename1)
	filename2 = directory+'/summary.h5'
	os.rename('summary.h5',filename2)
	# openmc.run(threads=64,mpi_args=['mpiexec', '-n', '8'])
